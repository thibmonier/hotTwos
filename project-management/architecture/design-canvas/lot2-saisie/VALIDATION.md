...
**Canevas** : 🎨 https://claude.ai/code/artifact/4b74613a-6890-428b-bb2a-950f751b17ba
(7 artboards ; sources versionnées dans `design-canvas/lot2-saisie/*.dc.html`.)

> ✅ **VALIDÉ PO — 100 % des écrans (2026-09-14, Thibaut).** Gate franchi → dev reskin (Sprint 21) débloqué. EPIC-013 bouclé.
...
|---|------------------|--------|-------------------------|-----------------------|---------|-------------|
| 1 | **DSH-COLLAB** (`Main.dc.html`) | Création | PageHeader (G4) + 4 StatCards (G1) contrepartie (heures/complétude/solde/avancement) ; CTA saisie 1 clic ; imputations récentes ; actions rapides | Conçu depuis parcours P1 ; **pas** de compteurs de volume ; gaps G1/G4 matérialisés + notés | ✅ **Validé PO** | |
| 2 | **PG-TMP-01 Saisie hebdo** (`SaisieHebdo.dc.html`) | Reskin+ | Grille projet × jours ouvrés ; totaux ligne/jour/semaine rendus ; barre d'objectif visible ; toggle week-ends ; confirmation « Enregistré » inline ; cibles 44px | TMP1-01 (totaux), TMP1-02 (objectif hors dialog), TMP1-03 (confirm inline), TMP1-04 (week-ends), TMP1-07 (code projet lisible) | ✅ **Validé PO** | |
| 3 | **PG-TMP-01 États** (`SaisieHebdoEtats.dc.html`) | Reskin+ | État vide (semaine vierge + CTA duplication) ; **état erreur de validation** (cellule > 24h, bordure + message + `aria-invalid` + bannière `role=alert` + enregistrement bloqué) | TMP1-05 (état erreur explicite, CA-4) | ✅ **Validé PO** | |
| 4 | **PG-TMP-01 Mobile** (`SaisieHebdoMobile.dc.html`) | Reskin+ | ~390px ; sélecteur de jours (statut par pastille) ; cartes projet empilées ; barre d'action collante ; pas de fausse status bar | CA-4 (variante mobile lisible) | ✅ **Validé PO** | |
| 5 | **PG-TMP-02/03 Saisie jour** (`SaisieJour.dc.html`) | Reskin+ | Mobile ; objectif du jour + total ; bannières **hors-ligne + resync** ; commentaire multi-ligne ; reprendre la veille ; cibles 44px+ | TMP2-02 (total), TMP2-03 (objectif jour), TMP2-04 (textarea), TMP2-01 (offline conservé) | ✅ **Validé PO** | |
| 6 | **PG-ABS-01 Absences** (`Absences.dc.html`) | Reskin+ | Compteurs (StatCards) ; formulaire + **impact solde projeté contextualisé** ; **vue calendrier** fériés/fermetures/conflits ; demi-journées reformulées ; badges statut icône+texte ; RGPD | ABS-01 (badges), ABS-02 (solde contextuel), ABS-03 (calendrier conflits), ABS-04 (demi-journées), ABS-05 (motif refus en détail), ABS-06 (RGPD) | ✅ **Validé PO** | |
| 7 | **PG-CPL-01 Complétude** (`Completude.dc.html`) | Reskin+ | E-mails (F-S5-4) ; badges texte+icône+couleur (F-S5-5) ; **StatCards synthèse** retard/partiel/soumis ; **relance inline** (sélection → relancer) ; filtre/recherche ; **colonne collaborateur figée** | CPL-01 (icônes), CPL-02 (e-mail, 🔴 bloquant), CPL-03 (StatCards G1), CPL-04 (relance inline), CPL-05 (filtre), CPL-06 (sticky) | ✅ **Validé PO** | |
...
|-------|------------------|------|-----------|---------|
| DSH-COLLAB | design-canvas v1 (2026-09-14) | | 2026-09-14 | Thibaut (PO) | ✅ Validé |
| Saisie hebdo | design-canvas v1 (2026-09-14) | | 2026-09-14 | Thibaut (PO) | ✅ Validé |
| Saisie hebdo — états | design-canvas v1 (2026-09-14) | | 2026-09-14 | Thibaut (PO) | ✅ Validé |
| Saisie hebdo — mobile | design-canvas v1 (2026-09-14) | | 2026-09-14 | Thibaut (PO) | ✅ Validé |
| Saisie jour | design-canvas v1 (2026-09-14) | | 2026-09-14 | Thibaut (PO) | ✅ Validé |
| Absences | design-canvas v1 (2026-09-14) | | 2026-09-14 | Thibaut (PO) | ✅ Validé |
| Complétude | design-canvas v1 (2026-09-14) | | 2026-09-14 | Thibaut (PO) | ✅ Validé |
